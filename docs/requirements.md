...
1. **API Documentation First**:
   - Start by writing your own API documentation for the given endpoint(s) — endpoints, methods, request/response structure, parameters, etc.
   - Use any format you're comfortable with (Markdown, a Postman collection, OpenAPI/Swagger, etc.).
   - **Do this first, before you start building.** This is a standalone deliverable — it shows us how you'd design/interpret a clean API structure.
   - _Note:_ You're free to rename endpoints/routes if you'd design them better, and to add or remove endpoints as you see fit — it's entirely your call.

2. **Feature Implementation**:
   Using the given mock data/API directly, build the following:
...
>
> _This bonus only applies if the addition is genuinely original. A common or generic addition won't count toward it, even if it's well executed._
...
>
> _This bonus only applies if the addition is genuinely original. A common or generic addition — a stock testimonial section, a standard FAQ accordion, and the like — won't count toward it, even if it's well executed._
...
- **Architecture & Technical Decisions**: Why you chose your architecture/libraries/approach in Part 1, and any trade-offs you considered.
- **Design Choices**: The reasoning behind your design choices in Part 2.
- **AI Tool Usage**: How you used AI tools (if at all) — which tool(s), what you used them for (e.g., boilerplate, debugging, drafting the API documentation, research), and what you changed, rejected, or wrote yourself instead of relying on the AI's output.
- **Future Improvements**: What you'd improve or do differently with more time.
...
| **API docs (Swagger)** | [https://frontend-task-chatapp.onrender.com/docs/](https://frontend-task-chatapp.onrender.com/docs/) |
